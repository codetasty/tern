...
(function(root, mod) {
  ...
  function findCompletions(srv, query, file) {
    ...
    let wordRegex = query.filterSequence ? new RegExp(word.split('').join('.*')) : null;
    ...
    function gather(prop, obj, depth, addInfo) {
      ...
      if (query.filter !== false && word) {
        if (query.filterSequence) {
          if (!wordRegex.exec(query.caseInsensitive ? prop.toLowerCase() : prop)) {
            return;
          }
        } else{
          if ((query.caseInsensitive ? prop.toLowerCase() : prop).indexOf(word) !== 0) {
            return;
          }
        }
      }
